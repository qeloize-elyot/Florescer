...
async function renderBrotos() {
  ...
  $("#meusResgates").innerHTML = disponiveis.length
    ? disponiveis.map((rg) => {
        ...
        return `<div class="passo">${r?.emoji || "🌱"} <strong>${r?.nome || rg.recompensaId}</strong> <span class="small muted">· disponível no carrinho</span></div>`;
      }).join("")
...
function renderCursos() {
  $("#gradeCursos").innerHTML = CURSOS.map((c) => {
    ...
    return `<article class="card">
      <div class="card-figura" style="font-size:4rem">${c.emoji}</div>
      <div class="card-corpo">
        ...
        <div class="painel">
          ...
          ${cursosFeitos.length ? cursosFeitos.map((c) => `
            <div class="passo" style="justify-content:space-between">
              <span>${c.emoji} <strong>${c.titulo}</strong> <span class="small muted">· ${c.progresso.codigo}</span></span>
